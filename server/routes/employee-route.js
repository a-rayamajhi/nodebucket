...
router.post("/:empId/tasks", async (req, res) => {
  try {
    Employee.findOne({ empId: req.params.empId }, function (err, employee) {
      if (err) {
        console.log(err);
        const createTaskMongoDbError = new BaseResponse('500', `MongoDB Exception: ${e.message}`)
        ...
        if (employee) {
          ...
          employee.save(function (err, updatedEmployee) {
            if (err) {
              console.log(err);
              const createTaskOnSaveMongoDbError = new BaseResponse('500', `MongoDB onSave() Exception: ${err.message}`)
          ...
          const invalidEmployeeIdResponse = new BaseResponse('200', `Invalid EmployeeId`)
    ...
    console.log(e)
    const createTaskException = new BaseResponse('500', `Internal Server Error: ${e.message}`)
...
router.get("/:empId/tasks", async (req, res) => {
  try {
    Employee.findOne({ 'empId': req.params.empId }, 'empId todo done', function (err, employee) {
      if (err) {
        console.log(err);
        const findAllTaskMongoDbError = new BaseResponse('500', `MongoDB Exception: ${e.message}`)
        res.status(500).send(findAllTaskMongoDbError.toObject())
      } else {
        console.log(employee)

        const findAllTaskOnSaveSuccessResponse = new BaseResponse('200', 'Successful query', employee);
    ...
    console.log(e)
    const findAllTaskException = new BaseResponse('500', `Internal Server Error: ${e.message}`)
...
router.put("/:empId/tasks", async (req, res) => {
  try {
      ...
      if (err) {
        ...
        if (employee) {
          ...
          employee.save(function (err, updatedEmployee) {
            if (err) {
              console.log(err);
              const updateTaskMongoDbError = new BaseResponse('500', `Internal Server Error ${e.message}`)
    ...
    console.log(e)
    const updateTaskException = new BaseResponse('500', `Internal Server Error: ${e.message}`)
...
router.delete("/:empId/tasks/:taskId", async (req, res) => {
  try {
    Employee.findOne({ 'empId': req.params.empId }, function (err, employee) {
      if (err) {
        console.log(err);
        const deleteTaskMongoDbError = new BaseResponse('500', `MongoDB Exception: ${e.message}`)
        ...
        if (employee) {
          ...
          if (todoItem) {
            ...
            employee.save(function (err, updatedTodoItemEmployee) {
              if (err) {
                console.log(err);
                const deleteTodoItemMongoDbError = new BaseResponse('500', `MongoDB Exception deleting TodoItem: ${e.message}`)
                res.status(500).send(deleteTodoItemMongoDbError.toObject())
              } else {
                console.log(updatedTodoItemEmployee)
                const deleteTodoItemSuccessResponse = new BaseResponse('200', 'Query successful', updatedTodoItemEmployee);
            ...
            employee.save(function (err, updatedDoneItemEmployee) {
              if (err) {
                console.log(err);
                const deleteDoneItemMongoDbError = new BaseResponse('500', `MongoDB Exception deleting DoneItem: ${e.message}`)
                res.status(500).send(deleteDoneItemMongoDbError.toObject())
              } else {
                console.log(updatedDoneItemEmployee)
                const deleteDoneItemSuccessResponse = new BaseResponse('200', 'Query successful', updatedDoneItemEmployee);
                res.status(200).send(deleteDoneItemSuccessResponse.toObject())
              }
            })
          } else {
            console.log(`invalid taskId! The passed-in value was ${req.params.empId}`)
            const invalidTaskIdResponse = new BaseResponse('200', `Invalid TaskId`)
    ...
    console.log(e)
    const deleteTaskException = new BaseResponse('500', `Internal Server Error: ${e.message}`)
